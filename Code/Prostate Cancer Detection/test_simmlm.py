"""
Evaluate a SimMLM (DMoME + MoFe) checkpoint over all 7 modality keep-subsets.

For each keep-subset S, dropped modalities are zeroed in the input AND the
gating presence vector is set accordingly so their gating weights are 0.
"""
...
from my_nnunet_cls.train_simmlm import (
    SimMLMDMoME, presence_from_inputs, apply_mask_to_inputs, _load_fold_split,
)


def load_checkpoint(ckpt_path: Path, device: torch.device, base: int, feat_dim: int,
                    dropout: float, gate_feat: int) -> SimMLMDMoME:
    model = SimMLMDMoME(base=base, feat_dim=feat_dim, dropout=dropout,
                        gate_feat=gate_feat).to(device)
...
@torch.no_grad()
def eval_one_combo(model: SimMLMDMoME, loader: DataLoader, device: torch.device,
    ...
    for (x0, x1, x2), y, _sid in pbar:
        ...
        for i in keep_set:
            keep_vec[i] = True
        mask_bool = pres & keep_vec.view(1, 3).expand(B, 3)
        mask = mask_bool.float()
        xs = apply_mask_to_inputs((x0, x1, x2), mask)
        logit, _, _ = model(xs, mask)
        loss = bce(logit, y)
...
def evaluate_all_combos(model: SimMLMDMoME, ds, device: torch.device,
...
def parse_args():
    ...
    p.add_argument("--out_json", type=str, default="eval_simmlm.json")
    return p.parse_args()
...
def main():
    ...
    payload = {"method": "simmlm", "fold": int(a.fold), "split": a.split,
               "ckpt": str(a.ckpt), "mod_names": mod_names,
               "target_shape": list(target_shape), **report}
