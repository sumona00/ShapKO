"""
Evaluate a PMR checkpoint over all 7 modality keep-subsets.
Dropped modalities are zeroed in the input. Inference uses the fused logit only.
"""
...
from my_nnunet_cls.train_pmr import PMRClassifier, _load_fold_split


def load_checkpoint(ckpt_path: Path, device: torch.device, base: int, feat_dim: int,
                    dropout: float, fusion_hidden: int) -> PMRClassifier:
...
class DropModalitiesWrapper(torch.utils.data.Dataset):
    def __init__(self, base_ds, dropped: List[int]):
...
@torch.no_grad()
def eval_one_combo(model: PMRClassifier, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    model.eval()
...
def evaluate_all_combos(model: PMRClassifier, ds, device: torch.device,
    ...
    for keep in keep_sets:
        ...
        loader = DataLoader(DropModalitiesWrapper(ds, dropped),
                            batch_size=batch_size, shuffle=False,
                            num_workers=num_workers, pin_memory=True, drop_last=False)
        m = eval_one_combo(model, loader, device)
        name = "+".join(mod_names[i] for i in keep) if keep else "NONE"
        results.append({"keep": keep, "dropped": dropped, "name": name, **m})
...
def parse_args():
    ...
    p.add_argument("--out_json", type=str, default="eval_pmr.json")
    return p.parse_args()
...
def main():
    ...
    payload = {"method": "pmr", "fold": int(a.fold), "split": a.split,
