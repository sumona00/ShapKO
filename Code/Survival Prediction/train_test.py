...
def load_checkpoint_trusted(path, device):
    # If you saved opt=argparse.Namespace and other python objects, you must use weights_only=False.
    ...
    for k, data in data_cv_splits.items():
        if k in fold_range:
            ...
            with torch.serialization.safe_globals([argparse.Namespace]):
                  model_ckpt = torch.load(load_path, map_location=device, weights_only=False)



            load_path = os.path.join(opt.checkpoints_dir, opt.exp_name, opt.model_name,
                         f"{opt.model_name}_{k}.pt")

            model_ckpt = load_checkpoint_trusted(load_path, device)
