...
random.seed(10)
def _safe_np_array(items):
    """
    If all items have identical shape -> stack into numeric array.
    If variable-length (e.g., patch bags) -> return object array.
    """
    if len(items) == 0:
        return np.array([])
    try:
        # Try strict stacking (works if same shape)
        return np.stack(items, axis=0)
    except Exception:
        # Variable-length => store as object
...
def complete_incomplete_data_selection(opt, data_cv_splits, data_cv_mask, available_idx_file_path, required_modality = ['rad','dna','demo','path']):
    ...
    for k, dataall in data_cv_splits.items():
        ...
        #data_cv_splits[k]['train']['x_path'] = _safe_np_array([dataall['train']['x_path'][x] for x in overlap_ID_list])
