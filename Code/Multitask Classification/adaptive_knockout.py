...
from mymodel.model_v3 import FlexCare
...
# Extra args: Shapley weighting + early stopping + TB + Adaptive KO
...
# Shapley-based loss weighting (NO effect on knockout/masks directly)
ko.add_argument(
...
# -------- Adaptive knockout (MCE-style) --------
ko.add_argument(
    '--ko_adaptive', action='store_true',
    help='Enable adaptive per-modality knockout rate (MCE-style).'
)
ko.add_argument(
    '--ko_min', type=float, default=0.02,
    help='Minimum knockout probability per modality.'
)
ko.add_argument(
    '--ko_max', type=float, default=0.5,
    help='Maximum knockout probability per modality.'
)
ko.add_argument(
    '--ko_gamma_presence', type=float, default=1.0,
    help='Exponent for balancing based on modality presence rate.'
)
ko.add_argument(
    '--ko_delta_importance', type=float, default=1.0,
    help='Exponent for balancing based on Shapley importance.'
)
ko.add_argument(
    '--ko_presence_ema', type=float, default=0.9,
    help='EMA factor for smoothing modality presence rates across epochs.'
)
...
def plot_shapley_and_rebal(
    ...
    Plot:
      1) Shapley importance for each modality vs. epoch, with a horizontal
         line at the base knockout rate.
      2) Rebalancing weights for each modality vs. epoch, also with that rate.
    """
    ...
    plt.axhline(y=knockout_rate, linestyle='--', label=f"Base KO rate={knockout_rate:.2f}")
    plt.xlabel("Epoch")
    plt.ylabel("Shapley importance")
    plt.title("Per-modality Shapley importance vs. base knockout rate")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    ...
    plt.title("Per-modality rebalancing weights vs. base knockout rate")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    # Base symmetric knockout derived from (1 - r)^d = 0.5
    base_knockout = clamp01(compute_feature_knockout_rate(num_modalities))  # ~0.2
    r = base_knockout  # keep name "r" for backwards-compatible logging
    ...
    # ---- Adaptive KO state (MCE-style) ----
    mods = ['ehr', 'cxr', 'note']

    # Global EMA of presence (fraction of samples with modality present)
    global_presence = {m: 1.0 for m in mods}

    # Current per-modality knockout rates
    current_mod_dropout = {m: base_knockout for m in mods}

    # Set on model
    if hasattr(model, "mod_dropout"):
        model.mod_dropout = current_mod_dropout.copy()
    model.feature_knockout_rate = base_knockout

    logger.info(
        f"[KO] Base modality-level knockout prob base_r = {base_knockout:.4f} "
        f"for d={num_modalities}."
    )
    if hasattr(model, "mod_dropout"):
        logger.info(f"[KO] Initial model.mod_dropout = {model.mod_dropout}")

    writer.add_scalar("KO/base_feature_knockout_rate", base_knockout, 0)
    for m in mods:
        writer.add_scalar(f"KO/r_{m}", current_mod_dropout[m], 0)
    ...
    for epoch in tqdm(range(1, args.epochs + 1), **tqdm_kwargs):
        ...
        # Track modality presence this epoch (for adaptive KO)
        epoch_present_sum = {m: 0.0 for m in mods}
        epoch_present_batches = 0
        ...
        for t_idx in range(len(mutli_train_dl)):
            ...
            with tqdm(mutli_train_dl[t_idx], position=0, ncols=120, **tqdm_kwargs) as tq:
                for _, data in enumerate(tq):
                    ...
                    # ----- track modality presence for KO adaptation -----
                    with torch.no_grad():
                        epoch_present_sum['ehr']  += mask_ehr_t.float().mean().item()
                        epoch_present_sum['cxr']  += mask_cxr_t.float().mean().item()
                        epoch_present_sum['note'] += mask_note_t.float().mean().item()
                        epoch_present_batches += 1
        ...
        logger.info(
            ...
            f"| mod_dropout={getattr(model, 'mod_dropout', {})}"
        )
        ...
        # ====== Validation: all modality combos (NO KO logic changed here; model.forward controls dropout) ======
        model.eval()
        ...
        if epoch >= shapley_start_epoch:
            ...
            logger.info(
                f"[SHAPLEY][Epoch {epoch:03d}] Importance (higher = stronger modality, ACC-based): "
            ...
            logger.info(
                ...
                f"(used for loss weighting next epoch): {current_rebal_weights} | base KO rate={r:.4f}"
            )
            ...
            logger.info(
                ...
                f"{last_shapley_importance} and rebal={current_rebal_weights} | base KO rate={r:.4f}"
            )
        ...
        # ====== Adaptive modality knockout (MCE-style) ======
        # Use global presence (EMA) + Shapley importance to update per-modality KO
        if args.ko_adaptive and epoch_present_batches > 0 and hasattr(model, "mod_dropout"):
            # 1) Epoch-level presence rates in [0,1]
            epoch_presence = {
                m: epoch_present_sum[m] / float(epoch_present_batches)
                for m in mods
            }

            # 2) Update global EMA presence
            ema = float(args.ko_presence_ema)
            for m in mods:
                global_presence[m] = (
                    ema * global_presence[m] + (1.0 - ema) * epoch_presence[m]
                )

            # 3) Compute adaptive knockout per modality
            rho_vals = np.array([global_presence[m] for m in mods], dtype=np.float64)
            rho_mean = float(rho_vals.mean() + 1e-8)

            shap_vals = np.array(
                [last_shapley_importance[m] for m in mods], dtype=np.float64
            )
            shap_mean = float(shap_vals.mean() + 1e-8)

            gamma = float(args.ko_gamma_presence)    # presence factor
            delta = float(args.ko_delta_importance)  # importance factor

            new_mod_dropout = {}
            for m in mods:
                # Presence factor: frequent modality (rho > rho_mean) -> factor > 1 -> more KO
                presence_factor = (global_presence[m] / rho_mean) ** gamma

                # Importance factor: strong modality (shap > shap_mean) -> factor > 1 -> more KO
                importance_factor = (last_shapley_importance[m] / shap_mean) ** delta

                # Combine: strong + frequent => higher KO; weak + rare => lower KO
                #r_m = base_knockout * presence_factor * importance_factor
                r_m = base_knockout * importance_factor
                r_m = clamp01(r_m)
                r_m = max(args.ko_min, min(args.ko_max, r_m))
                new_mod_dropout[m] = float(r_m)

            model.mod_dropout = new_mod_dropout
            # Optionally adjust scalar feature_knockout_rate as mean
            model.feature_knockout_rate = float(
                sum(new_mod_dropout[m] for m in mods) / len(mods)
            )

            logger.info(
                f"[KO][Epoch {epoch:03d}] Adaptive knockout updated | "
                f"presence={global_presence} | shapley={last_shapley_importance} | "
                f"mod_dropout={new_mod_dropout}"
            )

            for m in mods:
                writer.add_scalar(f"KO/r_{m}", new_mod_dropout[m], epoch)
                writer.add_scalar(f"KO/presence_{m}", global_presence[m], epoch)
        else:
            # Log presence even if KO is static
            for m in mods:
                writer.add_scalar(f"KO/presence_{m}", global_presence[m], epoch)
    ...
    plot_path = os.path.join('results_v2', 'val_loss_combos.png')
    plot_combo_losses(combo_history, plot_path)
    logger.info(f"[PLOT] Saved validation loss curves to {plot_path}")

    # ---- plot Shapley + rebal vs base knockout ----
    shap_plot, rebal_plot = plot_shapley_and_rebal(
        shapley_history, rebal_history, base_knockout, out_dir='results'
    )
